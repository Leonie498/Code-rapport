...
#-- Arguments (e.g. progress data) => change with your data
...
### use lcmm packages to initiate parameter values of longitunal sub-model
long_model <- lcmm::hlme(fixed = formFixed, 
                         random= formRandom,
                         subject = idVar, 
                         data=data)
priorMean.beta <- long_model$best[1:ncol(X)] #take the estimations for the mean of the prior
priorTau.beta <- diag(rep(precision,length(priorMean.beta))) # variance prior
...
if (param %in% c("value")) {
  ...
  K <- length(sk)
  P <- Time/2
  st <- outer(P, sk + 1) #multiplication of each element ok P with each element of sk (+1)
...
parms_to_save <- c("alpha", "alpha.beta","alpha.sigma", "beta", "covariance.b", "mu.log.sigma", "sigma2.log.sigma", "priorMean.b","alpha.assoc","shape","b","sigma")

## call JAGS using jagsUI

out_jags <- jagsUI::jags(data = jags.data,
                        parameters.to.save = parms_to_save  ,
                        model.file = "barrett.weibull.valeurcouranteJags.txt", #name of the model
                        #inits = param.inits,
                        n.chains = n.chains,
                        parallel = parallel,
                        n.adapt = n.adapt,
